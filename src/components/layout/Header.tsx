    ...
    <header className="sticky top-0 z-50 glass-nav w-full transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative px-4 py-2 rounded-md text-sm font-medium transition-all duration-300 font-sans tracking-wide cursor-pointer ${
                  isActive 
                    ? 'text-advocacy-gold' 
        ...
        <div className="hidden lg:flex items-center space-x-4">
          <button
            onClick={() => handleNavClick('pro-bono')}
            className={`flex items-center space-x-1.5 px-4 py-2 border rounded-full text-xs font-semibold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
              page === 'pro-bono'
